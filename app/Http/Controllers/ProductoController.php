...
class ProductoController extends Controller
{
    ...
    public function buscarProductos(Request $request,$id){
        ...
        $producto = Producto::select('productos.*')->where->where('productos.nombre','LIKE','%'.$request->nombre.'%')->where('productos.proveedor_id','=',$id)->get();
        return $producto;
    }
    ...
    public function marcasXProveedorCercano(Request $request){
        ...
        INNER JOIN productos p on marcas.id = p.marca_id WHERE negocios.id=:negociosId AND p.marca_id=:idMarca GROUP BY proveedors.id,marcas.nombre,nombreCompleto,distanciaKm ORDER BY distanciaKm ASC",array(
