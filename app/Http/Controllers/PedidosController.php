...
return $pedido;
    ...
    public function listaPedidosTotalesDelGerente($id){
        $pedidos = Pedidos::select('pedidos.*','proveedors.nombre','proveedors.apellido_paterno','proveedors.apellido_materno','productos.nombre as producto','productos.presentacion','productos.marca_id','marcas.nombre as marca')
        ->join('proveedors','proveedors.id','pedidos.proveedor_id')
        ->join('productos','productos.id','pedidos.producto_id')
        ->join('marcas','marcas.id','productos.marca_id')
        ->where('pedidos.gerente_id','=',$id)
